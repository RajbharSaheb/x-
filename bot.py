...
from lusifilms import web_server
...
async def lusifilms_start():
    ...
    for name in files:
        with open(name) as a:
            ...
            lusifilms_dir = Path(f"lusifilms/{plugin_name}.py")
            import_path = "lusifilms.{}".format(plugin_name)
            spec = importlib.util.spec_from_file_location(import_path, lusifilms_dir)
            load = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(load)
            sys.modules["lusifilms." + plugin_name] = load
